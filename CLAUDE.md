...
開發指令：`backend/` 跑 `npm run dev`（port 3001）、`frontend/` 跑 `npm run dev`（port 3000）。前端另有 `npm run lint`、`npm run build`；後端無 lint / test script。專案沒有自動化測試——API 手動測試流程見根目錄 `API_TESTING.md`（Postman 手冊）。
...
- `habits.is_archived`（封存）、`habits.visibility`（`'private'` | `'friends'`）、`habits.goal`（選填的可衡量目標，**僅本人可見——好友 API 回應必須剔除此欄位**）
...
- `routes/habits.js`：習慣 CRUD、週期、任務、打卡、週記事、統計（連續達標週數在程式端用 `utils/date.js` 的 `computeWeeklyStreak` 算：週執行率＝各任務完成次數以目標封頂加總 ÷ 目標加總，達 85% 即該週達標，取自《The 12 Week Year》）
- `routes/docs/`：auth 與 habits 的 API 端點文件（改 API 時同步更新）
...
- `design-system.md`：設計系統（sage 品牌色階、字體、間距）。**改 UI 前先讀**：顏色一律用 `app/_styles/globals.css` 的 CSS token（勿寫死 hex）；sage 為全站唯一 accent，`--streak` 暖色僅限 streak / 統計重點；選中狀態用白卡浮起（`bg-card` + `shadow-sm`）而非淡綠底
